...
### Model Selection With Missing Data Imputation
...
```{r}
#Testing  PR curves - ##class imbalance..? 914 vs 442..RESULT.....????
colnames(mydata_glm_cv_preds)
#creating truth class labels
mydata_glm_cv_preds <- mydata_glm_cv_preds %>%
  mutate(truth = imputed_data$cohort)  # Replace with your actual labels

autoplot(pr_curve(mydata_glm_cv_preds,
                  truth, 
                  .pred_1))

mydata_glm_cv_preds |>
  pr_curve(truth, .pred_1) |>
...
```{r}
$JUST FOR PRACTICE
# Calculate ROC and CI using pROC
lr_roc <- roc(mydata_glm_cv_preds$cohort, mydata_glm_cv_preds$.pred_0) #.pred_0? 1?
lr_ci <- ci.se(lr_roc, specificities = seq(0, 1, length.out = 100)) # Compute CIs
lr_ci

# Interpolate sensitivity values to align with `specificity_seq`
interp_sensitivity <- approx(
  x = 1 - lr_roc$specificities,  # Specificities from ROC curve
  y = lr_roc$sensitivities,      # Corresponding sensitivities
  xout = specificity_seq,        # Desired specificities for CI
  method = "linear"
)$y

# Construct the data frame for plotting
lr_roc_data <- data.frame(
  specificity = specificity_seq,
  sensitivity = interp_sensitivity,
  lower = lr_ci[, 1],
  upper = lr_ci[, 2]
)

# Define a sequence of specificities for confidence intervals
specificity_seq <- seq(0, 1, length.out = 100)
# Calculate confidence intervals for sensitivities at specificities
lr_ci <- ci.se(lr_roc, specificities = specificity_seq)
# Interpolate sensitivity values to align with specificity_seq
interp_sensitivity <- approx(
  x = 1 - lr_roc$specificities,  # Specificities from ROC curve
  y = lr_roc$sensitivities,      # Corresponding sensitivities
...
#F1 score????
f1_lr <- mydata_glm_cv_preds |>
  group_by(id) |>
  f_meas(truth = cohort, estimate = .pred_class, event_level = "second") # Ensure correct positive class
print(f1_lr)
# Average F1-Score across folds
average_f1 <- f1_lr %>% summarise(mean_f1 = mean(.estimate))
print(average_f1)
...
rf_preds <- collect_predictions(rf_fit_cv)
# Add a predicted class column based on the threshold (e.g., 0.5 for binary classification)
rf_preds <- rf_preds %>%
  mutate(.pred_class = ifelse(.pred_1 > 0.5, "1", "0"))
# Ensure cohort levels are correct
rf_preds <- rf_preds %>%
  mutate(cohort = factor(cohort, levels = c("0", "1")))

# Calculate F1-Score for each fold
f1_results_rf <- rf_fit_cv |>
  group_by(id) |>
  f_meas(truth = cohort, estimate = .pred_class, event_level = "second")
...
#Limitation 1. race -\> limited generalizability 2. relatively small size sample; however assessed imporant risk factors included, targetign CNCP with/witho OUD -\> still providing insights.

# Discussion

Random forest achieved the best predictive performance, with an area under the receiver operating characteristic curve of 0.89. Feature importance analysis identified nicotine dependence, age, and pain catastrophizing as the top three significant features. This model showed satisfactory prediction performance, and it can be a useful tool for clinicians to identify patients at risk for developing OUD and implement early intervention for prevention. Future studies are required to validate these findings externally. \# Conclusion
